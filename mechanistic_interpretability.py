...
class ModelPathwayAnalyzer:
    ...
    def get_thought_embeddings(self, question: str) -> Tuple[np.ndarray, str]:
        """Get layer-wise activations and generated response"""
        ...
        # Decode generated text (excluding original prompt)
...
def main():
    ...
    for cat_idx, (category, category_questions) in enumerate(questions.items(), 1):
        ...
        for q_idx, question in enumerate(category_questions, 1):
            try:
                ...
                # Get both embeddings and generated answer
    ...
    # Save all Q&A pairs with embeddings
    results_file = results_dir / "cognitive_traces.json"
    ...
    try:
        if all_answers:
            # Extract all embeddings from all answers
            combined_embeddings = np.concatenate([a["embeddings"] for a in all_answers])
            ...
            print("⚠️ No valid answers generated - skipping combined visualization")
    except Exception as e:
